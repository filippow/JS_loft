...
function collectDOMStat(root) {
    let obj = {
        ...
        takeInfo = function(element) {
            let el = element.tagName;

            if (el) {
                obj.tags[el] = obj.tags[el] ? ++obj.tags[el] : 1;

                // У текущего переданного элемента собираем информацию о классах. 
                for (let j=0; j< element.classList.length; j++) {
                    let cls = element.classList[j];

                    obj.classes[cls] = obj.classes[cls] ? ++obj.classes[cls] : 1;
                }
                
                // Если у элемента есть дочерние элементы то применяем к ним рекурсию
                if (element.childNodes.length >0) {
                    for (let i=0; i< element.childNodes.length; i++) {
                        takeInfo(element.childNodes[i]);
                    }
                } 
            } else {
                // Если tagName у элемента отсутствует, значит это обычный текстовый узел
                obj.texts++;
            }
        }

    // Запускаем функцию сбора информации внутри переданного root. Сам root не учитываем.
    for (let i=0; i<root.childNodes.length; i++) {
        takeInfo(root.childNodes[i]);
    }
